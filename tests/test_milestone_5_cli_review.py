...
class Milestone5CliReviewTest(unittest.TestCase):
    ...
    def test_review_shows_pending_only_by_default_and_all_with_flag(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            data_dir = Path(tmp_dir)
            (data_dir / "review_queue.json").write_text(
                json.dumps(
            ...
            with patch("sys.stdout", out_default):
                review_code = main(["review", "--data-dir", str(data_dir)])
            self.assertEqual(review_code, 0)
            default_lines = [line for line in out_default.getvalue().splitlines() if line.strip()]
            self.assertEqual(len(default_lines), 1)
            self.assertIn("rev_001", default_lines[0])

            out_all = StringIO()
            with patch("sys.stdout", out_all):
                review_all_code = main(["review", "--data-dir", str(data_dir), "--all-statuses"])
            self.assertEqual(review_all_code, 0)
            all_lines = [line for line in out_all.getvalue().splitlines() if line.strip()]
